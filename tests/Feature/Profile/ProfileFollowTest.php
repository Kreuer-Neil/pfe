...
test('a user can follow another user', function () {
    ...
    $response = $this->actingAs($user)->get(route('user-profile.follow', $target->id));
...
test('following the same user twice does not create a duplicate row', function () {
    ...
    $this->actingAs($user)->get(route('user-profile.follow', $target->id));
    $this->actingAs($user)->get(route('user-profile.follow', $target->id));
...
test('a user cannot follow themself', function () {
    ...
    $this->actingAs($user)->get(route('user-profile.follow', $user->id));
...
test('following a nonexistent user returns a 404', function () {
    ...
    $this->actingAs($user)
        ->get(route('user-profile.follow', 999999))
        ->assertNotFound();
});
...
test('a user can unfollow someone they follow', function () {
    ...
    $response = $this->actingAs($user)->get(route('user-profile.unfollow', $target->id));
...
test('unfollowing a nonexistent user returns a 404', function () {
    ...
    $this->actingAs($user)
        ->get(route('user-profile.unfollow', 999999))
        ->assertNotFound();
});
